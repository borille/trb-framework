 ...
 */
class Trb_View_Helper_Table extends Zend_View_Helper_Placeholder_Container_Standalone
{
    /**
     * @var string registry key
     */
    protected $_regKey = 'Trb_View_Helper_Table';
     ...
     * Class Constructor
     *
     * @param array|Zend_Db_Table_Rowset|Zend_Paginator|Zend_Db_Select|Trb_Db_Table_Abstract $data
     * @param array $columns
    ...
    {
        if ( is_object( $data ) ) {
            if ( get_parent_class( $data ) == 'Trb_Db_Table_Abstract' ) {
                $this->setPrimaryKey( $data->getPrimary() );
                $data = $data->getSelect();
            }

            if ( get_class( $data ) == 'Zend_Db_Select' ) {
                $data = Trb_Action_Helper::paginator( $data, Zend_Controller_Front::getInstance()->getRequest()->getParam( 'page' ) );
    ...
    protected function getPrimaryKeyParams()
    {
        if ( !$this->_primaryKeyParams ) {
            if ( !$this->_primaryKey ) {
                throw new Exception( 'N�o foi definido a PK da tabela!' );
            }
